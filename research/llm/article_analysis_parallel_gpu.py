...
# Check if running on Apple Silicon and MPS support is available
if torch.backends.mps.is_available():
    device = torch.device("mps")  # Use Apple's GPU (M1/M2)
    print(f"Using device: {device} (Apple M1/M2 GPU with MPS)")
else:
    device = torch.device("cpu")  # Fallback to CPU if no MPS is available
